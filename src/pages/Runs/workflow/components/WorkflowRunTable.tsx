import { useWorkflowRunDataModel } from '@/api/workflow';
...
const WorkflowRunTable = () => {
  ...
  const fullSubjectModel = useWorkflowRunDataModel({
  ...
  },
  {
    ...
    cell: (status: unknown) => {
      if (!status) {
        ...
        return (
          <Badge status={status as string} className='flex flex-row h-8'>
            {/* <StatusIcon status={status as string} className='h-4 w-4 text-white pr-2'></StatusIcon> */}
            {status as string}
          </Badge>
        );
